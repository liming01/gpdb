...
class ConnectionInfo(object):
    ...
    @staticmethod
    def get_hostname_port(content_id, role_name):
...
class SQLIsolationExecutor(object):
    ...
    class SQLConnection(object):
        ...
        def query(self, command, out_sh_cmd, in_sh_cmd, global_sh_executor):
            ...
            # if set @in_sh
            if in_sh_cmd != None:
                (hostname, port) = ConnectionInfo.get_hostname_port(int(self.name), 'p')
...
if __name__ == "__main__":
    ...
    f = open("/home/muchen/repo/gpdb/src/test/isolation2/input/parallel_retrieve_cursor/status.source", "r")
    executor.process_isolation_file(f, sys.stdout, options.initfile_prefix)