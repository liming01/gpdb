...
static void *exec_parallel_cursor_threadfunc(void *master_conn)
{
...
main(int argc, char **argv)
{
	...
	pgoptions_retrieve_mode = "-c gp_session_role=retrieve";			/* specify this connection is in the retrieve mode */
	...
	if(exec_sql_without_resultset(master_conn,"DROP TABLE IF EXISTS public.tab_parallel_cursor;")!=0) goto LABEL_ERR;
	if(exec_sql_without_resultset(master_conn,"CREATE TABLE public.tab_parallel_cursor AS SELECT id FROM pg_catalog.generate_series(1,100) id;")!=0) goto LABEL_ERR;

	/* start a transaction block because parallel cursor only support WITHOUT HOLD option */
	if(exec_sql_without_resultset(master_conn,"BEGIN;")!=0) goto LABEL_ERR;

	/* declare parallel cursor for this table */
	if(exec_sql_without_resultset(master_conn,"DECLARE myportal PARALLEL CURSOR FOR select * from public.tab_parallel_cursor;")!=0) goto LABEL_ERR;
	...
	/* create retrieve mode connection to endpoints according to the endpoints info fetched */
	for (i = 0; i < ntup; i++)
	{
		if(i==0)
		{
			/* currently all endpoints of one parallel cursor has the same token, so just set it at the first time */
	...
	pthread_t thread1;
	/* create a second thread to execute parallel cursor because it will waiting until all data finished retrieved */
	...
	/* call it to suspend the main thread, so that the thread of "execute the parallel cursor" will run fistly */
	usleep(1);
	
	/* 
	 * Waiting for the status to becomes 'READY', and then retrieve the result of the endpoints
	 * This section can be execute parallely on different host or in different threads/processes on the same host.
	 * For simplicity, here just use loop in one process.
	 */
...
LABEL_RETRY:
	...
	/* close the cursor */
	if(exec_sql_without_resultset(master_conn,"CLOSE myportal;")!=0) goto LABEL_ERR;

	/* end the transaction */
	if(exec_sql_without_resultset(master_conn,"END;")!=0) goto LABEL_ERR;
